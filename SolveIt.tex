...
This is the \textbf{Windows 11} edition of the simulator (x86-64; the
processor must have FMA --- every Intel chip since 2013 and AMD since
2012 does). You need Rust with the MSVC toolchain: download and run
\code{rustup-init.exe} from \code{https://rustup.rs}; it installs
\code{x86\_64-pc-windows-msvc} by default and will offer the Visual
Studio C++ Build Tools if none are present (any Visual Studio 2022 or
2026 install already satisfies it --- Rust only needs its linker).
Then, in PowerShell:
...
Checking your build: \code{cargo test -{}-workspace} --- 622 tests
should pass (567 unit and integration tests plus 55 documentation
tests), measured on Windows~11 Pro for Workstations 25H2, rustc 1.91.1,
target \code{x86\_64-pc-windows-msvc}.
...
(Rust's own \code{\{:e\}} formats exponents differently), and the
transcendental functions were taken off the host entirely. The vendored
engine is the \textbf{Windows port} of the translation: because
Microsoft's math runtime (UCRT) rounds \code{sin}, \code{cos},
\code{exp}, \code{ln} and friends differently from the Linux library the
reference outputs came from, the engine carries its own pure-Rust
translations of the glibc routines (\code{sundials\_libm}), each
measured at \textbf{0 mismatches over millions of inputs} against a real
glibc oracle. That --- plus a deterministic \code{pow} and a pinned FMA
instruction (\code{.cargo/config.toml}) --- is why this Windows build
reproduces the Linux physics \textbf{byte for byte}: the 12 collision
scripts, all 13 recorded videos and 57 of the 59 dynamic notebooks are
byte-identical to the Linux evidence, and the remaining two (the quantum
notebooks, whose first-party code still uses the host libm) agree to the
last printed digit (see \code{evidence/win11/}).
...
equilibrium/sensitivity, 112 language, 92
