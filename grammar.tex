...
The machine mode speaks the same language over JSON (the transcript is
Git Bash; PowerShell piping into
\code{.\textbackslash{}target\textbackslash{}release\textbackslash{}posim.exe}
produces the same replies):
...
no crates.io dependency. It is the \textbf{Windows 11 / x86-64 port} of
the translation
(\code{once-ere/SUNDIALS\_7\_8\_Rust\_port\_for\_Windows11}), which
differs from the Linux port in exactly one substantive way: Microsoft's
math runtime (UCRT) rounds \code{sin}, \code{cos}, \code{exp},
\code{ln} and the inverse/hyperbolic functions differently from the
Linux glibc that generated the reference outputs, so the Windows port
carries its own pure-Rust translations of the glibc routines
(\code{sundials\_libm}), each measured at 0 mismatches over millions of
inputs against a real glibc oracle. Its deterministic routines use the
fused multiply-add instruction, so the workspace pins
\code{-C target-feature=+fma} in \code{.cargo/config.toml} (any Intel
CPU since 2013 or AMD since 2012 has it). The upgrade from the 7.7.0
engine the predecessor repository used is recorded, with its evidence,
in \code{PORT\_7.8.0\_PROVENANCE.md}; the Windows port of this whole
simulator is recorded in \code{PORT\_WIN11\_PROVENANCE.md}.
...
\code{recorder/src/record\_video.py} makes one:
