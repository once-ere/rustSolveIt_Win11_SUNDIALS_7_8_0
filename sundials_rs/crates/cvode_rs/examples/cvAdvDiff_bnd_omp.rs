/* -----------------------------------------------------------------
 * Ported from: examples/cvode/C_openmp/cvAdvDiff_bnd_omp.c
 * -----------------------------------------------------------------
 * Example problem:
 *
 ...
 *
 * This version uses the OpenMP N_Vector. The number of threads is taken from
 * OMP_NUM_THREADS, or from the first command-line argument when one is given,
 * exactly as the C does through omp_get_max_threads().
 *
 * The thread count changes the printed values: the five sum reductions in the
 * OpenMP vector accumulate a private partial per thread and then combine them,
 * so the order of the additions — and the last bits of the result — depend on
 * how many threads there are. `sundials_libm`-style determinism does not
 * apply here; what the port reproduces is the arithmetic of OpenMP's
 * `schedule(static)` for the given thread count. See `nvector_openmp`.
 * -----------------------------------------------------------------*/
...
use cvode_rs::nvector_openmp::N_VNew_OpenMP;
...
fn main() {
    ...
    The C starts at 1, overwrites it with omp_get_max_threads() when built
    with OpenMP — which is OMP_NUM_THREADS if that is set, otherwise the
    processor count — and then lets argv[1] override. This reproduces that
    order of precedence. */
    ...
    if let Ok(s) = std::env::var("OMP_NUM_THREADS") {
        ...
        num_threads = n.get() as i32;
    }
    let argv: Vec<String> = std::env::args().collect();
    if argv.len() > 1 {
        /* overwrite with command line value, if supplied */
        num_threads = SUNStrToReal(&argv[1]) as i32;
    }

    /* Create an OpenMP vector */

    let u = N_VNew_OpenMP(NEQ, num_threads, &sunctx_h); /* Allocate u vector */
    if check_retval(SUNContext_GetLastError(&sunctx_h), "N_VNew_OpenMP") != 0 {
        std::process::exit(1);
    }
    let u = u.expect("N_VNew_OpenMP");
