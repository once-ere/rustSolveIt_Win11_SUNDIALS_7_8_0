...
cd "$(dirname "$0")/.."
WS_ROOT="$(pwd)"
# The upstream read-only SUNDIALS 7.8.0 C tree. The sibling ports keep the
# workspace *inside* that tree, so "$WS_ROOT/.." is the historical default.
# This port lives outside it (it is its own repository under .../github/),
# so the location is taken from $SUNDIALS_C_TREE when that is set.
UPSTREAM="${SUNDIALS_C_TREE:-$WS_ROOT/..}"
...
LOGS="$WS_ROOT/logs"
mkdir -p "$LOGS"
# Windows-native (x86_64-pc-windows-msvc) example binaries carry .exe.
EXE=""
...
verify_crate() {
  ...
  parse_cmake "$cml" | while IFS='|' read -r name args outfile; do
    ...
    local bin="$WS_ROOT/target/release/examples/$name$EXE"
    if [ ! -x "$bin" ]; then
